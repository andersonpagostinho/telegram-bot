    ...
    try:
        ...
        if confirmado_flag:
            ...
            if not resultado_lock.get("ok"):
                motivo = resultado_lock.get("motivo", "Erro ao criar evento")
                tipo_erro = resultado_lock.get("tipo_erro", "desconhecido")
                print(f"❌ Evento não pôde ser criado ({tipo_erro}): {motivo}")
                return f"conflito_{tipo_erro}"

            if resultado_lock.get("duplicado"):
            ...
            # 🔥 EVENTO NÃO CONFIRMADO: salvar normalmente em memória temporária
            # (Estes ficam em MemoriaTemporaria e não ocupam agenda)
            # Não usar lock aqui, pois não são definitivos

            # ANTIDUPLICIDADE básica
            existente = await buscar_dado_em_path(path)
            if existente:
                print("♻️ Evento pendente já existe. Não criando duplicado.")
                return "duplicado"
