...
"""
SMOKE TEST — Cenário Mais Perigoso Pós-Commit P0

Objetivo:
Validar que o fix P0 não quebrou em cenário de máxima complexidade:
- Fluxo ativo de agendamento
- Interrupção com pergunta lateral
- Retomada com escolha de profissional

Fluxo:
1. "quero agendar corte"      → inicia agendamento (servico=corte)
2. "amanhã"                   → preenche data
3. "às 10"                    → preenche hora
4. "quanto custa escova?"     → PERGUNTA LATERAL (não deve alterar draft)
5. "Bruna"                    → escolhe profissional (deve continuar com corte)

Validações críticas:
✅ Passo 1: servico='corte'
✅ Passo 3: data_hora preenchida
✅ Passo 4: pergunta NÃO altera servico (continua corte)
✅ Passo 5: profissional vinculado ao corte, não a escova
✅ Nenhum evento criado (aguardando confirmação)
✅ Não entra em onboarding
✅ Não muda para consulta pura
"""
...
async def main():
    ...
    print("\n" + "=" * 90)
    print("SMOKE TEST — Cenário Mais Perigoso Pós-Commit P0")
    print("=" * 90)
    ...
    mensagens = [
        ...
        {"texto": "quanto custa escova?", "esperado": {"servico": "continua_corte"}},
    ...
    for i, msg_data in enumerate(mensagens, 1):
        ...
        # Capturar passo 4 para validação
        if i == 4:
            ctx_passo_4 = copy.deepcopy(ctx_atual)
        ...
        elif i == 4:  # Pergunta lateral crítica
            if resumo["servico"] != "corte":
                falhas.append(f"Passo 4 (CRÍTICO): pergunta lateral ALTEROU servico para {resumo['servico']!r}, deveria continuar 'corte'")

            draft = resumo["draft_agendamento"] or {}
            if draft.get("servico") != "corte":
                falhas.append(f"Passo 4: draft servico alterado para {draft.get('servico')!r}")

        elif i == 5:  # Escolha de profissional
            if resumo["profissional_escolhido"] != "Bruna":
                falhas.append(f"Passo 5: profissional deveria ser 'Bruna', obtido {resumo['profissional_escolhido']!r}")

            if resumo["servico"] != "corte":
                falhas.append(f"Passo 5: servico deveria ser 'corte', obtido {resumo['servico']!r}")
    ...
    resultado_path = Path(__file__).parent / "resultado_smoke_cenario_perigoso.json"
    with open(resultado_path, "w", encoding="utf-8") as f:
        json.dump({
            "nome": "smoke_cenario_perigoso",
            "descricao": "Fluxo completo com interrupção lateral e escolha de profissional",
            "status": status,
            "falhas": falhas,
            "ctx_final": resumo_ctx(ctx_final),
            "ctx_passo_4": resumo_ctx(ctx_passo_4) if ctx_passo_4 else None,
