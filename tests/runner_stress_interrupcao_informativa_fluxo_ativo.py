...
"""
STRESS RUNNER — RAJADA COM INTERRUÇÃO INFORMATIVA E FLUXO ATIVO

Objetivo:
Validar que pergunta informativa NÃO interrompe o fluxo de agendamento
quando endereço já existe (onboarding já foi completado).

Fluxo:
1. "quero corte"        → inicia agendamento (servico = corte)
2. "qual o endereço?"   → pergunta informativa (NÃO é alteração)
3. "amanhã"             → continua agendamento (data)
4. "às 10"              → continua agendamento (hora)

Validações:
- Após passo 1: servico == "corte" ou draft.servico == "corte"
- Após passo 2: serviço/draft NÃO foram limpos (pergunta informativa não apaga contexto)
- Após passo 4: data_hora contém "10:00"
- estado_fluxo em ["agendando", "aguardando_profissional", "aguardando_confirmacao_agendamento"]
- NÃO criar evento
- NÃO chamar fluxo de onboarding (endereço já mockado)
"""
...
def validar_interrupcao_informativa(ctx_final, ctx_passo_2, falhas_acumuladas):
    """Validar que pergunta informativa não interrompe fluxo ativo"""
    ...
    if servico != "corte" and draft.get("servico") != "corte":
        falhas.append(f"serviço não iniciado: esperado 'corte', obtido servico={servico!r}, draft={draft.get('servico')!r}")

    # Validação pós-passo 2: contexto NÃO foi limpo pela pergunta informativa
    if ctx_passo_2:
...
async def main():
    ...
    # Mensagens da rajada
    mensagens_rajada = [
        {"texto": "quero corte", "descricao": "Inicia fluxo com serviço"},
        {"texto": "qual o endereço?", "descricao": "Pergunta informativa (NÃO deve limpar)"},
        {"texto": "amanhã", "descricao": "Continua agendamento com data"},
        {"texto": "às 10", "descricao": "Continua agendamento com hora"},
    ]
    ...
    print("\n" + "=" * 90)
    print("STRESS RUNNER - INTERRUÇÃO INFORMATIVA COM FLUXO ATIVO")
    print("=" * 90)
    ...
    resultado_path = Path(__file__).parent / "resultado_stress_interrupcao_informativa_fluxo_ativo.json"
    resultado_completo = {
        "nome": "interrupcao_informativa_fluxo_ativo",
        "descricao": "Pergunta informativa não deve interromper fluxo quando endereço existe",
