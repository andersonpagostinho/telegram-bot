...
"""
STRESS RUNNER — MENSAGENS MULTILINHA EM UMA ENTRADA

Objetivo:
Validar que o sistema processa corretamente mensagens com quebras de linha
sem perder entidades ou interpretar apenas a primeira linha:
- serviço + data + hora em linhas diferentes
- serviço + profissional + hora em linhas diferentes
- profissional + data + hora em linhas diferentes
- tudo junto em múltiplas linhas
- confirmação multilinha
"""
...
def validar_caso(caso, resultado, mensagens, ctx_final, erro):
    ...
    if exp.get("espera_confirmacao"):
        # Apenas verificar que a confirmação foi processada sem erro
        if erro:
            falhas.append(f"confirmação multilinha falhou: {erro['mensagem']}")
        return {
            "nome": caso["nome"],
...
async def main():
    ...
    casos = [
        {
            "nome": "multilinha_servico_data_hora",
        ...
        {
            "nome": "multilinha_servico_profissional_hora",
            "mensagem": "escova\ncom Bruna\nàs 14",
            "espera": {
                "servico": "escova",
                "profissional": "Bruna",
        ...
        {
            "nome": "multilinha_profissional_data_hora",
            "mensagem": "com Gloria\namanhã\nàs 15",
            "espera": {
                "profissional": "Gloria",
                "hora": "15:00",
        ...
        {
            "nome": "multilinha_confirmacao",
            "mensagem": "sim\npode confirmar",
            "espera": {
                "espera_confirmacao": True,
            },
        },
    ]
    ...
    print("\n" + "=" * 90)
    print("STRESS RUNNER - MENSAGENS MULTILINHA")
    print("=" * 90)
    ...
    for i, caso in enumerate(casos, 1):
        ...
        print(f"\nCASO {i}: {caso['nome']}")
        print(f"   Mensagem: {repr(caso['mensagem'][:50])}{'...' if len(caso['mensagem']) > 50 else ''}")
        ...
        try:
            ...
            # Encoding error é sintoma, não a causa — log mas continua
            print(f"[AVISO] Encoding error durante rota (ignorado): {e}")
            resultado = None
        ...
        status_symbol = "OK" if validacao["status"] == "SUCESSO" else "FAIL"
    ...
    resultado_path = Path(__file__).parent / "resultado_stress_multilinha_agendamento.json"
