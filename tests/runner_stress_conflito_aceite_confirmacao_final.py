...
"""
STRESS RUNNER — CONFLITO → ACEITE → CONFIRMAÇÃO → CRIAÇÃO

Objetivo:
Validar o fluxo COMPLETO:
1. Cliente pede mudança que entra em conflito
2. Sistema sugere alternativa
3. Cliente aceita sugestão
4. Sistema oferece confirmação com sugestão aceita
5. Cliente confirma
6. Sistema cria evento com dados corretos

Caso crítico:
"na verdade quero escova às 10" → conflito (detecta conflito)
"pode ser" → aceita sugestão 10:40 (reconstrói com 10:40)
"sim" → confirma (cria evento com escova às 10:40)
...
from services import event_service_async
from services import firebase_service_async
...
class TelegramUpdateMock:
    def __init__(self, user_id):
        self.message = TelegramMessageMock()
        self.message.from_user = TelegramUserMock(user_id)
        self.message.text = ""
...
class ContextoMock:
    ...
    async def salvar_contexto_temporario(self, user_id, contexto):
        # 🔑 Substituição COMPLETA, não merge parcial
        # Isso garante que pop() e exclusões sejam respeitadas
...
def validar_caso(caso, resultado, mensagens, ctx_final, erro, evento_criado=None):
    evento_criado = evento_criado or {}
    falhas = []
    ...
    if caso["nome"] == "conflito_aceite_confirmacao_final":

        if ctx_final.get("servico") != "escova":
            falhas.append(
                f"ctx final: serviço incorreto, esperado escova, obtido {ctx_final.get('servico')!r}"
            )

        if ctx_final.get("profissional_escolhido") != "Bruna":
            falhas.append(
                f"ctx final: profissional incorreto, esperado Bruna, obtido {ctx_final.get('profissional_escolhido')!r}"
            )

        if ctx_final.get("data_hora") != "2026-06-05T10:40:00":
            falhas.append(
                f"ctx final: data_hora incorreta, esperado 2026-06-05T10:40:00, obtido {ctx_final.get('data_hora')!r}"
            )

        if evento_criado.get("descricao") != "Escova com Bruna":
            falhas.append(
                f"evento criado: descrição incorreta, esperado 'Escova com Bruna', obtido {evento_criado.get('descricao')!r}"
            )

        if evento_criado.get("profissional") != "Bruna":
            falhas.append(
                f"evento criado: profissional incorreto, esperado Bruna, obtido {evento_criado.get('profissional')!r}"
            )

        # Validar data e hora combinadas
        evento_data = evento_criado.get("data")
        evento_hora = evento_criado.get("hora_inicio")
        esperada_data = "2026-06-05"
        esperada_hora = "10:40"

        if evento_data != esperada_data:
            falhas.append(
                f"evento criado: data incorreta, esperado {esperada_data}, obtido {evento_data!r}"
            )

        if evento_hora != esperada_hora:
            falhas.append(
                f"evento criado: hora incorreta, esperado {esperada_hora}, obtido {evento_hora!r}"
            )

        if evento_criado.get("descricao") == "Corte com Bruna":
            falhas.append(
                "evento foi criado com descrição ANTIGA (Corte com Bruna)"
            )

        if evento_hora == "08:20":
            falhas.append(
                "evento foi criado com hora ANTIGA (08:20)"
            )

    return {
        "nome": caso["nome"],
        "mensagem": caso.get("mensagem") or " | ".join(caso.get("mensagens", [])),
...
async def main():
    ...
    # Definir todos os mocks ANTES de usar
    async def verificar_conflito_mock(*args, **kwargs):
        return []  # Sem conflito

    async def buscar_dado_em_path_mock(*args, **kwargs):
        return None  # Não existe

    async def salvar_dado_em_path_mock(*args, **kwargs):
        return True

    async def buscar_cliente_mock(user_id):
        return {"nome": "Cliente Teste"}

    async def criar_notificacao_agendada_mock(*args, **kwargs):
        return True
    ...
    # Registrar mocks em pr (principal_router)
    ...
    # Mockar salvar_evento (função real que persiste em Firestore)
    async def salvar_evento_mock(user_id: str, evento: dict, event_id: str = None):
        evento_criado.clear()
        evento_criado.update(evento)
        print(f"[MOCK] salvar_evento chamado com evento: {evento}")
        return True

    # Registrar mocks em event_service_async
    event_service_async.salvar_evento = salvar_evento_mock
    event_service_async.verificar_conflito = verificar_conflito_mock
    event_service_async.buscar_dado_em_path = buscar_dado_em_path_mock
    event_service_async.salvar_dado_em_path = salvar_dado_em_path_mock
    event_service_async.buscar_cliente = buscar_cliente_mock
    event_service_async.criar_notificacao_agendada = criar_notificacao_agendada_mock

    # Mockar verificar_pagamento
    async def verificar_pagamento_mock(*args, **kwargs):
        return True

    async def verificar_acesso_modulo_mock(*args, **kwargs):
        return True

    # Mockar buscar_eventos_por_intervalo (precisa ser async)
    async def buscar_eventos_por_intervalo_mock(*args, **kwargs):
        return []

    # Registrar mocks em event_handler (onde add_evento_por_gpt os chama)
    event_handler.salvar_evento = salvar_evento_mock
    event_handler.verificar_conflito_e_sugestoes_profissional = conflito_mock
    event_handler.buscar_eventos_por_intervalo = buscar_eventos_por_intervalo_mock
    event_handler.validar_horario_funcionamento = validar_horario_funcionamento_mock
    event_handler.verificar_pagamento = verificar_pagamento_mock
    event_handler.verificar_acesso_modulo = verificar_acesso_modulo_mock

    # Registrar básicos de contexto
    ...
    casos = [
        {
            "nome": "conflito_aceite_confirmacao_final",
            "mensagens": [
                "na verdade quero escova às 10",
                "pode ser",
                "sim",
            ],
            "espera": {
                "servico": "escova",
                "profissional": "Bruna",
                "data_hora": "2026-06-05T10:40:00",
                "evento_criado": True,
            },
        },
    ]

    # CASOS ANTIGOS (comentados para referência)
    # =====================================================
    """
        {
    ...
    print("\n" + "=" * 90)
    print("🧪 STRESS RUNNER — CONFLITO → ACEITE → CONFIRMAÇÃO")
    print("=" * 90)
    ...
    for i, caso in enumerate(casos, 1):
        ...
        resultado = None
        erro = None
        telegram_context = TelegramContextMock(user_id=user_id)
        telegram_update = TelegramUpdateMock(user_id=user_id)

        print("\n" + "-" * 90)
        print(f"CASO {i:02d}: {caso['nome']}")

        lista_mensagens = caso.get("mensagens", [caso.get("mensagem")])

        for msg_idx, texto_msg in enumerate(lista_mensagens, 1):
            print(f"MSG {msg_idx}: {texto_msg!r}")
            telegram_update.message.text = texto_msg
    ...
    caminho = Path(__file__).parent / "resultado_stress_conflito_aceite_confirmacao_final.json"
