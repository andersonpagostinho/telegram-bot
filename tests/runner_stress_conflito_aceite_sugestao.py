...
"""
STRESS RUNNER — CONFLITO → ACEITE → CONFIRMAÇÃO

Objetivo:
Validar o fluxo completo de mudança com conflito:
1. Cliente pede mudança que entra em conflito
2. Sistema sugere alternativa
3. Cliente aceita sugestão
4. Sistema reconstrói agendamento com alternativa aceita

Caso crítico:
"na verdade quero escova às 10" → conflito
"pode ser" → aceita sugestão 10:40
Esperado: escova com Bruna às 10:40
...
def validar_caso(caso, resultado, mensagens, ctx_final, erro):
    ...
    if caso["nome"] == "conflito_aceite_sugestao":

        if ctx_final.get("servico") != "escova":
            falhas.append(
                f"serviço não foi atualizado para escova: obtido {ctx_final.get('servico')!r}"
            )

        if ctx_final.get("profissional_escolhido") != "Bruna":
            falhas.append(
                f"profissional não mantido: esperado Bruna, obtido {ctx_final.get('profissional_escolhido')!r}"
            )

        if ctx_final.get("data_hora") != "2026-06-05T10:40:00":
            falhas.append(
                f"horário não foi atualizado para sugestão: esperado 2026-06-05T10:40:00, obtido {ctx_final.get('data_hora')!r}"
            )

        if ctx_final.get("aguardando_confirmacao_agendamento") is not True:
            falhas.append(
                "não aguardando confirmação após aceitar sugestão"
            )
