...
async def tratar_mensagens_gerais(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "code=" in texto:
        try:
            ...
            code = None
    elif texto.startswith("4/"):
        code = texto

    if code:
        print(f"⚠️ Código de autenticação detectado mas função 'auth_callback' não existe mais: {code}")
        await update.message.reply_text("🚫 Autenticação por código desativada no momento.")
    else:
        print("🚀 Entrou no processar_texto()")
        await processar_texto(update, context)
        print("📨 Vai chamar processar_texto()")
