...
from handlers.acao_handler import tratar_mensagem_gpt as tratar_mensagem_usuario, pegar_sessao
...
async def processar_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if servico_mencionado and not intencao_listagem_ampla:
        ...
        profissionais_filtrados = profissionais  # 🔄 Usa a lista completa
    ...
    if (
        ...
        "não há profissionais" in resultado.get("resposta", "").lower()
    ):
        profissionais_formatados = [
            f"- {p['nome']}: {', '.join(p['servicos'])}" for p in contexto["profissionais"]
        ]
        resultado["resposta"] = "Aqui estão os profissionais cadastrados:\n" + "\n".join(profissionais_formatados)
        resultado["acao"] = None
        resultado["dados"] = {}
