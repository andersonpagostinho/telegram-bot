...
def identificar_intencao(texto):
    ...
    comandos = {
        ...
        "consultar_preco_servico": [
            "quanto custa", "qual o valor", "preço de", "valor do", "quanto sai", 
            "valor da consulta", "quanto é", "quanto fica", "custa quanto"
        ],
...
def deve_ativar_fluxo_manual(mensagem: str) -> bool:
    msg = mensagem.lower()
    return (
        "profissionais" in msg and
        any(p in msg for p in ["dia", "às", "as", "hora", "horário"]) and
        any(p in msg for p in ["corte", "cortar", "escova", "manicure"])
    )