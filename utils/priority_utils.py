...
def classificar_prioridade_email(email_data: dict, user_id: str = "") -> str:
    ...
    try:
        ...
        # ⚠️ Bloquear domínios promocionais comuns
        DOMINIOS_PROMOCIONAIS = [
            "temuemail.com", "info.centauro.com.br", "inspire.pinterest.com",
            "magazine", "shopee", "mercadolivre"
        ]
        remetente_email_match = re.search(r"<(.+?)>", email_data.get('remetente', ''))
        email_puro = remetente_email_match.group(1) if remetente_email_match else email_data.get('remetente', '').lower()
        dominio = email_puro.split("@")[-1] if "@" in email_puro else ""
        if any(d in dominio for d in DOMINIOS_PROMOCIONAIS):
            return "baixa"
