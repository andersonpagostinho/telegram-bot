...
def resolver_proximo_passo_real(
    ...
    if (
        ...
        or contexto.get("horarios_sugeridos")
    ):
        ...
        if not tem_servico_ctx:
            return "perguntar_servico"

        if not tem_profissional_ctx:
...
async def resolver_alteracao_draft_agendamento(
    ...
    # =====================================================
    if alteracao.get("tipo") == "servico":
        novo_servico = alteracao.get("valor")

        if not novo_servico:
            return await _send_and_stop(
                context,
                user_id,
                "Qual serviço você quer colocar no lugar?",
                parse_mode=None
            )
        ...
        if not valido.get("ok"):
            ...
            return await _send_and_stop(
                ...
                user_id,
                (
                    f"{profissional} não atende {novo_servico}.\n\n"
        ...
        if conflito.get("conflito"):
            ...
            if sugestoes:
                ...
                return await _send_and_stop(
                    ...
                    user_id,
                    (
                        f"Consigo trocar para {novo_servico}, mas esse horário não encaixa.\n\n"
                        f"Tenho esta opção: {sugestoes[0]}.\n\n"
                        "Quer esse horário?"
                    ),
                    parse_mode=None
            ...
            return await _send_and_stop(
                ...
                user_id,
                (
                    f"Consigo trocar para {novo_servico}, mas esse horário não está disponível.\n\n"
                    "Quer que eu procure outro horário?"
                ),
                parse_mode=None
            )
        ...
        return await _send_and_stop(
            ...
            (
                f"Perfeito 😊\n\n"
                f"{novo_servico.capitalize()} com {profissional}\n"
                f"📆 {data_legivel}\n\n"
                "Posso confirmar?"
            ),
            parse_mode=None
...
async def roteador_principal(user_id: str, mensagem: str, update=None, context=None):
    ...
    preservar_confirmacao_pendente = (
    ...
    if eh_confirmacao_pendente_ativa(ctx) and (
        ...
        # 🔥 P0: Bloquear placeholder sem hora confirmada
        eh_placeholder = (
            str(data_hora or "").endswith("T00:00:00")
            and (
                ctx.get("data_sem_hora") is True
    ...
    if (
        ...
        if (
            ...
            if servico_slot and profissional_escolhido and data_hora_slot:
                ...
                # =====================================================
                if profissional_escolhido and servico_slot and data_hora_slot:
    ...
    if estado_fluxo in ("aguardando_servico", "aguardando serviço", "aguardando_serviço"):
        ...
        # ✅ PRÉ-CHECK OFICIAL: validação + conflito + confirmação
        return await precheck_e_confirmacao_agendamento(
            context=context,
            user_id=user_id,
            ctx=ctx,
    ...
    if not acao and proximo_passo_real:
        ...
        if proximo_passo_real == "perguntar_servico":
            ...
            horarios_ctx = ctx.get("horarios_sugeridos") or []
            ...
            if horarios_ctx:
                ...
                return await _send_and_stop(
                    ...
                    (
                        f"Perfeito 😊\n\n"
                        f"Para *{ctx.get('servico')}* com *{ctx.get('profissional_escolhido')}*, "
                        f"tenho *{opcoes}*.\n\n"
                        "Qual horário você prefere?"
                    )
                )
